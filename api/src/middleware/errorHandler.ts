 ...
 */
function errorHandlerMiddleware(error: Error, request: Request, response: Response, next: NextFunction) {
 ...
 */
function getHttpStatusCode(options: { error: any, response: Response }): number {
	/**
	 * Check if the error object specifies an HTTP
	 * status code which we can use.
	 */
	const statusCodeFromError = options.error.status || options.error.statusCode;
	 ...
	 */
	const statusCodeFromResponse = options.response.statusCode;
