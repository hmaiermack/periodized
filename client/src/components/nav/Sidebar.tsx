...
interface ISidebarProps {
    ...
    programList: Array<IProgramPreview>
}

interface IProgramPreview {
    _id: string,
    name: string
}
...
const Sidebar = ({username, currentProgramId, programList}: ISidebarProps) => {
            ...
            <nav>
                <ul className="space-y-2 text-sm">
                    ...
                    <div className="flex flex-col p-2 rounded-md font-medium ">
                        <div className="flex">
                            <span className="text-left">Your Programs</span>
                        </div>
                        <div className="flex flex-col p-2 font-sm text-gray-600">
                            {programList && 
                                programList.slice(0, 3).map((program) => (
                                    <NavLink to={`/programs/${program._id}`} key={program._id} className="hover:bg-gray-200 rounded-md p-2 focus:shadow-outline break-words">
                                        {program.name}
                                    </NavLink>
                                ))
                            }
                            ...
                            <div className={`w-full flex flex-col font-sm text-gray-600 first:pt-0 `}>
                                {programList && programList.length > 2 &&
                                    programList.slice(3).map((program) => (
                                        <NavLink to={`/programs/${program._id}`} key={program._id} className="hover:bg-gray-200 rounded-md p-2 focus:shadow-outline break-words">
                                            {program.name}
                                        </NavLink>
                                    ))
